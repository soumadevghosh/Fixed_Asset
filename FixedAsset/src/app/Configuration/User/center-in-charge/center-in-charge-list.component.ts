...
import { CenterInChargeDialogComponent } from './center-in-charge-dialog.component';

@Component({
  selector: 'app-center-in-charge-list',
  templateUrl: './center-in-charge-list.component.html',
  providers: [UserModel, UserService]
})
export class CenterInChargeListComponent implements OnInit {
    ...
    this.service.getUser().subscribe((response) => {
      if (response.isIdentityExist) {
        if (response.isSuccess) {
          this.dataSource = new MatTableDataSource(response.userList.filter(filter => filter.centerInChargeDeptId > 0));
  ...
  openDialog() {
    ...
    this.dialog.open(CenterInChargeDialogComponent, dialogConfig)
