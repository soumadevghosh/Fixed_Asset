...
@Component({
  selector: 'app-unit-inventory-dept-dialog',
  templateUrl: './unit-inventory-dept-dialog.component.html',
  providers: [UserModel, UserService]
})
export class UnitInventoryDeptDialogComponent implements OnInit {
  Title:string;
  Units: any[];
  UnitInventoryDepartments: any[];
  UserForm: FormGroup;
  constructor(private service: UserService, public model: UserModel, private toastr: ToastrService,
    public dialogRef: MatDialogRef<UnitInventoryDeptDialogComponent>, @Inject(MAT_DIALOG_DATA) public dialogData: UserModel,
  ...
  ngOnInit(): void {
    if (this.dialogData != null) {
      ...
      this.UnitInventoryDepartments = this.dialogData.inventoryDeptList;
  ...
  setFormData(dialogData: any) {
    ...
    this.UserForm.controls.inventoryDeptName.patchValue(dialogData.inventoryDepId);
  ...
  onSubmit(UserForm: any) {
    ...
    this.model.userType=this.model.users[1].value;
