...
import { AccountsDeptDialogComponent } from './accounts-dept-dialog.component';

@Component({
  selector: 'app-accounts-dept-list',
  templateUrl: './accounts-dept-list.component.html',
  providers: [UserModel,UserService]
})
export class AccountsDeptListComponent implements OnInit {
    ...
    this.service.getUser().subscribe((response) => {
      if (response.isIdentityExist) {
        if (response.isSuccess) {
          this.dataSource = new MatTableDataSource(response.userList.filter(filter => filter.accountsDeptId > 0));
  ...
  openDialog() {
    ...
    this.dialog.open(AccountsDeptDialogComponent, dialogConfig)
