...
@Component({
  selector: 'app-accounts-dept-dialog',
  templateUrl: './accounts-dept-dialog.component.html',
  providers: [UserModel,UserService]
})
export class AccountsDeptDialogComponent implements OnInit {

  constructor(private service: UserService, private model: UserModel, private toastr: ToastrService,
    private dialogRef: MatDialogRef<AccountsDeptDialogComponent>, @Inject(MAT_DIALOG_DATA) public dialogData: UserModel,
  ...
  onSubmit(UserForm: any) {
    if (UserForm.invalid) {
      ...
      this.model.userType=this.model.users[3].value;
