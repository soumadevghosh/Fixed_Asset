...
@Component({
  selector: 'app-procurement-executive-dialog',
  templateUrl: './procurement-executive-dialog.component.html',
  providers: [UserModel, UserService]
})
export class ProcurementExecutiveDialogComponent implements OnInit {

  constructor(private service: UserService, private model: UserModel, private toastr: ToastrService,
    private dialogRef: MatDialogRef<ProcurementExecutiveDialogComponent>, @Inject(MAT_DIALOG_DATA) public dialogData: UserModel,
  ...
  onSubmit(UserForm: any) {
    if (UserForm.invalid) {
      ...
      this.model.userType=this.model.users[2].value;
